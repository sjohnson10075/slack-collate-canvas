...
bolt.shortcut("export_pdf", async ({ ack, shortcut, client, logger }) => {
    ...
    async function compressToJpeg(buf) {
    ...
    for (const [i, it] of items.entries()) {
        ...
        if (orig) {
            try {
                ...
                page.drawText("[image processing failed]", { x, y: afterCaptionY - lineHeight, size: captionSize, font, color: (0, pdf_lib_1.rgb)(0.4, 0, 0) });
            }
        }
        else {
    ...
    const up = (await client.apiCall("files.getUploadURLExternal", { filename, length: byteLen }));
    if (!up?.ok) {
    ...
    if (!externalUploaded) {
        ...
        const up2 = await client.files.uploadV2({
            channel_id,
            thread_ts: root_ts,
            filename,
